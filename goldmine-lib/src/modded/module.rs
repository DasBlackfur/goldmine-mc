...
use anyhow::Result;
use mlua::{Lua, RegistryKey, Table, Value::{self, Nil}};
...
pub fn goldmine_module(lua: &Lua, registries: Arc<Mutex<RefCell<Registries>>>) -> Result<RegistryKey> {
    ...
    let register_mod = lua.create_function(move |lua, lua_mod: Table| {
        ...
        registry_handle.lock().borrow_mut().lm_registry.register(&mod_name, new_mod);
...
fn registry_module(lua: &Lua, registries: Arc<Mutex<RefCell<Registries>>>) -> Result<Table> {
    ...
    registry_module.set("pl_registry", registry_functions(lua, registries.clone(), "pl_registry".to_owned())?)?;

    Ok(registry_module)
}

fn registry_functions(lua: &Lua, registries: Arc<Mutex<RefCell<Registries>>>, registry_name: String) -> Result<Table<'_>> {
    ...
    let values_func = lua.create_function(move |lua, ()| {
        ...
        Ok(registry.values().map(|key| lua.registry_value(key).unwrap()).collect::<Vec<Value>>())
    })?;
